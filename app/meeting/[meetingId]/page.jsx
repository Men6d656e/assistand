"use client";
import MeetingRoom from "@/app/components/meating-room";
...
const MeetingPage = () => {
  ...
  useEffect(() => {
    setUser({
      id: name.toLocaleLowerCase().replace(/\s+/g, "-"), // Akash -> akash
      name,
    });
  }, [name]);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-900 text-white">
        <div className="p-6 bg-red-900/20 border border-red-500 rounded-lg">
          ...
          <p> {error}</p>
          <button
            className="mt-4 px-4 py-2 bg-red-500 rounded-lg hover:bg-red-500"
            onClick={() => router.push("/")}
          >
            Back
          </button>
